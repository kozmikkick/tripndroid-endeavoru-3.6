...
unsigned long usb_net_raw_ip_intf[MAX_INR] = { 0x03, 0x05, 0x07 };
...
static struct baseband_usb *baseband_usb_net[MAX_INR] = { 0, 0, 0};

static struct net_device *usb_net_raw_ip_dev[MAX_INR] = { 0, 0, 0};

static unsigned int g_usb_interface_index[MAX_INR];
static struct usb_interface *g_usb_interface[MAX_INR];
...
static struct usb_device_id baseband_usb_driver_id_table[MAX_INR][2];

static char baseband_usb_driver_name[MAX_INR][32];

static struct usb_driver baseband_usb_driver[MAX_INR] = {
	{
...
static int usb_net_raw_ip_init(void)
{
	...
	for (i = 0; i < max_intfs; i++) {
		...
		eth_random_addr(usb_net_raw_ip_dev[i]->dev_addr);
